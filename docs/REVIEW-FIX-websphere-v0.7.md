...
- 展开后 → 完整表单 + `▲ 收起` 按钮
- 状态记忆到 `otb:last:websphere:target_collapsed`（collapsed / expanded），跨刷新保持
...
| 10 | 刷新页面，折叠状态记忆 | expanded → reload → 仍展开；collapsed → reload → 仍折叠；localStorage key `otb:last:websphere:target_collapsed` 正常 | ✅ 全过 |
...
| 摘要刷新时用 renderTargetSummary 闭包变量 sysSel/getSelectedTargets/credStatus，会不会 stale？| ✅ 都是 OTB.core 闭包内引用 + 每次调用读最新值，OK |
...
| localStorage key 命名冲突？| ✅ 用 OTB.core.lastSet 走 `otb:last:websphere:` 前缀，跟其它 sel/cred 等命名空间一致 |
