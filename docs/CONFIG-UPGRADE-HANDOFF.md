...
- 同一数据目录同时只能有一个升级事务。
- 互斥由操作系统内核文件锁保证：Unix 用非阻塞排他 `flock`，Windows 用 `LockFileEx`
  （立即失败模式），实现在 `internal/sysutil/filelock.go`（平台分支
  `filelock_unix.go` / `filelock_windows.go`）。
- 持锁进程被杀死后内核会自动释放锁，因此**不需要**任何"超时接管"策略，
  也不需要在 release 时 unlink 锁文件：锁文件路径保持稳定，release 只解锁并关闭句柄，
  避免新 inode 绕过旧 inode 上的锁。
- 锁文件内记录的 pid / token / 起始时间只作为**诊断信息**，不再参与互斥判定。
  历史上"超过 15 分钟视为遗留锁可自动接管"的做法已废弃：它通过读取旧 PID 判断进程是否
  死亡，再删除锁文件重建，两个进程可能同时成功持锁（一个进程删掉另一个刚创建的新锁）。
- 由于 Windows 上被锁定的区间对其他句柄不可读，诊断信息在锁释放后才可读取，属预期行为。
...
- 非关键文件损坏只警告且不覆盖
- 损坏内部升级清单重建
- 内核文件锁互斥（活跃持锁者阻止后来者；持锁进程被杀后锁自动释放，无需手工删锁文件）
- 锁内诊断信息（pid/token/起始时间）写入
- 快照恢复成功
- 恢复前安全快照
- 快照篡改拒绝且当前文件不变
- 资产清单完整性
