...
// popup_windows.go：自绘右下角悬浮通知。
//
// 设计要点：
//   - 圆角用 SetWindowRgn(CreateRoundRectRgn)，跨 Win 7/10 都支持（无需 DWM）。
//   - 半透明用 WS_EX_LAYERED + LWA_ALPHA（整体 alpha，不依赖 UpdateLayeredWindow）。
//   - 文字用 GDI DrawTextW（无抗锯齿，但简洁够用；后续可升级 GDI+）。
//   - 关闭按钮：自己处理 WM_LBUTTONDOWN，按坐标判断关闭按钮区域。
//   - 多条提醒排队显示，避免堆叠。
//   - 主题跟随系统：注册表 HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize
//     下 AppsUseLightTheme = 0 表示深色（Win10 1809+）；Win 7 没有此值，默认浅色。
...
const (
	...
	// 配色（深色主题）—— 与 popup-dark-1-indigo 背景图风格一致
...
func onPaint(hwnd uintptr, content string) {
	...
	// 2) 左侧 6px accent 条（垂直渐变：上浅下深，比纯色更有质感）
...
// 上端：colAccent 原色；下端：colAccent 暗化 30%。
//
// GradientFill 用两个 TRIVERTEX + 一个 GRADIENT_RECT，垂直渐变方向 (GRADIENT_FILL_RECT_V)。
// 跨 Win 7 / Win 10 都支持（msimg32.dll / gdi32 都有）。
...
func runPopupWindow(item popupItem) {
	...
	className, _ := windows.UTF16PtrFromString(popupClassName)
	winName, _ := windows.UTF16PtrFromString(item.content)
