...
func (m *Manager) UpdatePatch(enabled, windows *bool, wecom, ding *string) error {
	...
	if windows != nil {
		cfg.WindowsToast = *windows
	}
	if wecom != nil {
		cfg.WeComWebhook = *wecom
	}
	if ding != nil {
		cfg.DingWebhook = *ding
	}
	return m.Update(cfg)
}
...
func (m *Manager) NotifyFailure(event schedtask.FailureEvent) {
	if event.RunID == "" {
		event.RunID = event.TaskID + ":" + event.StartedAt
	}
	m.mu.Lock()
	if _, ok := m.seen[event.RunID]; ok {
		m.mu.Unlock()
		return
	}
	...
	if cfg.WeComWebhook != "" {
		go m.postWebhook(cfg.WeComWebhook, map[string]any{"msgtype": "text", "text": map[string]string{"content": message}})
	}
	if cfg.DingWebhook != "" {
		go m.postWebhook(cfg.DingWebhook, map[string]any{"msgtype": "markdown", "markdown": map[string]string{"title": "Kairo 定时任务失败", "text": strings.ReplaceAll(message, "\n", "\n\n")}})
	}
}
...
func (m *Manager) Test() error {
	...
	cfg := m.cfg
	m.mu.RUnlock()
	if !cfg.WindowsToast && cfg.WeComWebhook == "" && cfg.DingWebhook == "" {
		return errors.New("尚未配置任何告警通道")
	}
	if cfg.WindowsToast {
		popup.Show("Kairo 告警通道测试\n这是一条测试通知")
	}
	var first error
	items := []struct {
		url  string
	...
	for _, item := range items {
		...
		if err := m.postWebhookSync(item.url, item.body); err != nil && first == nil {
			first = err
		}
	}
	return first
...
func (m *Manager) postWebhook(raw string, body any) {
	for i := 0; i < 3; i++ {
		if err := m.postWebhookSync(raw, body); err == nil {
			return
		}
...
func (m *Manager) postWebhookSync(raw string, body any) error {
	...
	req, err := http.NewRequest(http.MethodPost, raw, bytes.NewReader(encoded))
	if err != nil {
		return err
	}
	...
	if len(response) > 64<<10 {
		return errors.New("Webhook 响应超过 64 KiB")
	}
	// WeCom and DingTalk can reject a delivery while returning HTTP 200.
	// Keep accepting generic webhook responses that do not use this envelope.
	var result map[string]json.RawMessage
	if json.Unmarshal(response, &result) == nil {
		if rawCode, exists := result["errcode"]; exists {
			var code int
			if err := json.Unmarshal(rawCode, &code); err != nil || string(rawCode) == "null" {
				return errors.New("Webhook 返回无效 errcode")
			}
			if code != 0 {
				var message string
				_ = json.Unmarshal(result["errmsg"], &message)
				return fmt.Errorf("Webhook 业务失败 (errcode=%d): %s", code, message)
			}
		}
	}
	return nil
