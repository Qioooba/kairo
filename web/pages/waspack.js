...
(function () {
  ...
  async function renderWASPack(view) {
    ...
    async function showRebuild(item) {
      ...
      try {
        ...
        submit.onclick = async function () {
          ...
          try {
            const result = await api('POST', '/api/waspack/history/' + encodeURIComponent(item.id) + '/rebuild', { output_dir: outputDir, output_policy: policy.value || 'fail', confirm_replace: confirmReplace, include_zip: !!include.checked });
    ...
    async function doPreview(seq) { if (!validate()) { showAlert('请先修正配置中的必填项。', 'warn'); return; } const result = await api('POST', '/api/waspack/preview', payload()); if (state.disposed || seq !== operationSeq) return; renderPreview(result); toast('预检完成', result.missing && result.missing.length ? 'warn' : 'ok'); }
    async function doBuild(includeZip, seq) { if (!validate()) { showAlert('请先修正配置中的必填项。', 'warn'); return; } const body = payload(), preview = await api('POST', '/api/waspack/preview', body); if (state.disposed || seq !== operationSeq) return; renderPreview(preview); if (preview.missing && preview.missing.length) return; const confirmed = await askReplace(body.output_dir, '一键生成', 'full'); if (body.output_policy === 'replace' && !confirmed) { phase.textContent = '已取消覆盖。'; return; } body.include_zip = !!includeZip; body.confirm_replace = !!confirmed; let response; try { response = await api('POST', '/api/waspack/build', body); } catch (error) { if (includeZip && error && error.data && error.data.build) { renderResult({ build: error.data.build, zip: null, partial: true, error: error.data.error || errorText(error) }, '投产包已生成，但 ZIP 失败。'); showAlert('投产包已生成，但 ZIP 生成失败：' + (error.data.error || errorText(error)), 'warn'); loadHistory(true); return; } throw error; } if (state.disposed || seq !== operationSeq) return; renderResult(normalizeBuildResponse(response), includeZip ? '投产包与 ZIP 均已生成。' : '投产包已生成。'); toast(includeZip ? '投产包与 ZIP 已生成' : '投产包已生成', 'ok'); loadHistory(true); }
    async function doExtract(seq) { if (!validate()) { showAlert('请先修正配置中的必填项。', 'warn'); return; } const body = payload(), confirmed = await askReplace(body.output_dir, '抽取 WAR', 'full'); if (body.output_policy === 'replace' && !confirmed) { phase.textContent = '已取消覆盖。'; return; } body.confirm_replace = !!confirmed; setStage(''); const response = await api('POST', '/api/waspack/extract', body); if (state.disposed || seq !== operationSeq) return; setStage(response && response.stage_token); renderExtracted(response); toast('WAR 已抽取', 'ok'); rememberProject(body.project_dir); loadHistory(true); }
    async function doPackage(seq) { if (!validate()) { showAlert('请先修正配置中的必填项。', 'warn'); return; } if (!state.stageToken || state.stageStale) { showAlert('当前 WAR 抽取凭据缺失或已过期，请重新抽取。', 'warn'); return; } const body = payload(); body.stage_token = state.stageToken; const confirmed = await askReplace(body.output_dir, '打包 WAR', 'artifacts'); if (body.output_policy === 'replace' && !confirmed) { phase.textContent = '已取消覆盖。'; return; } body.confirm_replace = !!confirmed; const response = await api('POST', '/api/waspack/package', body); if (state.disposed || seq !== operationSeq) return; renderResult(normalizeBuildResponse(response), '已按 war 当前内容生成投产包。'); toast('WAR 打包完成', 'ok'); loadHistory(true); }
    async function doZip(seq) { if (!validateZip()) { showAlert('请先修正 ZIP 所需的输出目录和包名。', 'warn'); return; } const body = payload(), confirmed = await askReplace(body.output_dir, '生成 ZIP', 'artifacts'); if (body.output_policy === 'replace' && !confirmed) { phase.textContent = '已取消覆盖。'; return; } body.confirm_replace = !!confirmed; const response = await api('POST', '/api/waspack/zip', body); if (state.disposed || seq !== operationSeq) return; renderResult(normalizeBuildResponse(response), 'ZIP 已生成。'); toast('ZIP 已生成', 'ok'); loadHistory(true); }
  ...
  Kairo.waspackTest = { canonicalStageSignature, windowsPathKey, sameWindowsPath, comparableHistoryItem, previousComparable, normalizeBuildResponse, makeHandoffPayload, createHandoff, handoffTargetURL, HANDOFF_PREFIX };
})();
