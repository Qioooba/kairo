...
[![Status](https://img.shields.io/badge/Status-v0.20-success)](#)
...
产品版本只改根目录 `VERSION`（例如 `v0.20`）。运行时版本由 `go:embed VERSION` 进入 `/api/config`，前端页脚和关于页只读这个字段。
...
| 版本 | 功能 |
| --- | --- |
| **v0.20** | 多Tab保活与GBK自适应 + 数据库LOB与事务终端状态防护 + 代码比对行级对齐与视角保持 + 崩溃恢复日志与原子升级 + 工业级全量测试与安全加固 (正式版) |
| **v0.19** | 工作台深化与安全加固（数据库网格写入/批量导入/LOB流式分级 + 跨平台敏感路径拦截 + 来源版本联动防覆盖） |
...
### v0.20（当前正式版）— 多Tab保活 · GBK中文字符自适应 · 数据库与比对深化 · 崩溃恢复与原子升级 · 工业级测试闭环

#### 多Tab独立保活与GBK全链路自适应
- **多Tab保活与生命周期管理**：各工作台具备独立资源所有权，支持子路由（subroute）状态同步、未保存草稿保护与 beforeunload 拦截，切换页面零重载、零白屏、零状态丢失。
- **GBK 中文字符全链路自适应**：Windows 环境下全面兼容中文目录、中文文件名、SFTP 路径解析与父目录（`..`）回溯，自适应字符集探测与转码，彻底杜绝乱码与路径解析异常。

#### 数据库工作台可信行定位与事务终端状态保护
- **UPDATE 投影可信定位**：数据库网格编辑与行更新前置可信行定位器（trusted locator）校验，杜绝复杂投影与并发下的误更新风险。
- **事务终端状态严格锁定**：事务执行具备 terminal 状态追踪，防止事务提交或回滚后意外覆写或状态漂移。
- **导出完整上下文传递**：导出 CSV/Excel 自动携带会话上下文、参数与筛选条件，保证导出数据与屏幕呈现严格一致。
- **CLOB/BLOB 与大结果集流式分级**：大字段采用预览、令牌与流式下载分离设计，避免超大字段打爆内存。

#### 代码比对工作台交互跃升
- **横向平滑滚动与光标选区保持**：优化水平滚动联动与光标选中文本状态保持，杜绝跳屏与重绘闪烁。
- **保存退回机制与行级对齐覆盖**：支持比对保存后一键退回历史状态，勾选对齐行覆盖策略更精准。
- **白名单默认优化**：比对白名单默认留空无限制，便于本机与内网快速比对。

#### 系统升级自愈与工业级测试闭环
- **崩溃恢复日志（Recovery Journal）**：升级协调器引入 journal 事务日志与启动预检自愈机制，即使升级期间遭遇断电或异常杀进程，重启后自动自愈，杜绝半更新脏数据。
- **75 项标准验收矩阵（T001-T075）**：自动化端到端测试全覆盖，涵盖数据库、比对、WAS 打包、便笺、通知、安全 RBAC、CORS/WebSocket 校验与性能基线。

完整设计原则、架构分层、42 项分级修复与迁移说明见应用内「关于 → 版本演进史 → v0.20」。

### v0.19 — 工作台深化 + 安全边界收紧 + LOB 与批量事务

这是从 v0.18 迈向 v0.20 的关键深化版本：
...
- **测试与发布边界**：扩充数据库写入、LOB、会话隔离、比较任务、WAS 历史/路径和前端工作台测试。

完整设计原则、架构调整、修复与迁移注意见应用内「关于 → 版本演进史 → v0.19」。

### v0.18 — 工作台闭环 + 跨文件升级协调 + 19项深度修复 + 自动化测试全绿
